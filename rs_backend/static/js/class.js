       ...
       // alert(JSON.stringify({"action":"add","display_name":display_name,"description":description,"name":name,"order":order}))
        $.ajax({
            ...
            error: function(response){
                jQuery('#addClassModal').removeClass('open');
    ...
    $.ajax({
        ...
        success: function(response){
            if (response.status ==true){
                ...
                var class_saved = "Success Fully Saved"
                var display_name = $('#title').val(response.data[0]['display_name'])
                var description = $('#description').val(response.data[0]['description'])
                var name = $('#shortcode').val(response.data[0]['name'])
...
function updateclass()
{
    ...
    var order = $('#order').val()
    var  cl_id = $('#hidden_class_id').val()
    dataString  = {"action":"Update","name":name,"display_name":display_name,"description":description,"order":order,"id": cl_id}
