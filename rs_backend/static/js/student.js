    ...
    data.paginations = JSON.stringify(paginations);
              ...
              success: function(response){
                  if (response.status != false) {
                      for(var i=0;i<response.data.length;i++){
                          $('#divisionFilter').append(`<option value='${response.data[i].id}'>${response.data[i].division}</option>`);
                          department_count+=1;
                      }
                  }
      ...
      $.ajax({
              ...
              success: function(response){
                  if (response.status != false) {
                      for(var i=0;i<response.data.length;i++){
                          $('#subject-getting-started').append(`<option value="${response.data[i].id}">${response.data[i].name}</option>`);
                      } 
                      $('#subject-getting-started').selectpicker('refresh');
                  }
