...
from subjects.models import SubjectHasUnit
...
class Programs(models.Model):
    ...
    type = models.IntegerField(choices=subject_choice,blank=True, default=1)
    created_on = models.DateTimeField(default=datetime.now, blank=True)
    target = models.ForeignKey(Target, related_name='target_name',  on_delete=models.CASCADE)
    # created_by = models.IntegerField()
...
class Programclasshassubjects(models.Model):
    ...
    class_has_subjects = models.ForeignKey(SubjectHasUnit, related_name='subject_has_unit', on_delete=models.CASCADE)

class Sessions(models.Model):
    YEAR_CHOICES = [(r, r) for r in range(2000, date.today().year + 1)]
    created_on = models.DateTimeField(default=datetime.now, blank=True)
    # created_by = models.IntegerField()
    #program = models.ForeignKey(Programs, related_name='session_programs', on_delete=models.CASCADE)
    name = models.CharField(max_length=120)
    display_name = models.CharField(max_length=120)
    description = models.TextField()
...
class SessionHasProgram(models.Model):
    # created_by =  models.IntegerField()
    session = models.ForeignKey(Sessions,related_name='sessions_has_program',  on_delete=models.CASCADE)
...
class PhaseHasSession(models.Model):
    ...
    display_name = models.CharField(max_length=120)
    description = models.TextField()
    program_session = models.ForeignKey(SessionHasProgram, related_name='phase_session', on_delete=models.CASCADE)
